...
class imgToPdf_converter:
    def __init__(self, root):
        self.root = root
        self.image_paths = []
        self.output_pdf_name = tk.StringVar()

        self.select_image_listbox = tk.Listbox(root, selectmode=tk.MULTIPLE)
        self.initialize_ui()
    ...
    def initialize_ui(self):
        ...
        self.select_image_listbox.pack(pady=(0, 10),
        ...
        pdf_name_entry = tk.Entry(
            self.root, textvariable=self.output_pdf_name,
            width=40, justify="center")
        pdf_name_entry.pack()

        convert_pdf_button = tk.Button(
            self.root, text="Convert to PDF",
            command=self.convert_image_to_pdf)
        convert_pdf_button.pack(pady=(20, 40))
    ...
    def update_selected_images_listbox(self):
        self.select_image_listbox.delete(0, tk.END)

        for image_path in self.image_paths:
            filename = os.path.basename(image_path)
            self.select_image_listbox.insert(tk.END, filename)
...
def main():
    ...
    converter = imgToPdf_converter(root)
    root.mainloop()
